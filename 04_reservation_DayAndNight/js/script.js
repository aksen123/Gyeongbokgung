...
select_item.addEventListener("change", () => {
  ...
  window.history.pushState(
    null,
    null,
    `?dn=${urlParams.get("dn")}&event=${urlParams.get("event")}`
  );
...
function totalPrice() {
  ...
  document.querySelector(".total-person").innerText =
    +public.innerText + +discount.innerText;
  //결제금액
...
function payment() {
  ...
  if (userName.value.length < 2 || +userName.value === NaN) {
    check_bool = false;
    userName.focus();
  }
  if (
...
function completeReservation() {
  ...
  let eventSelectValue =
    eventSelect.options[eventSelect.selectedIndex].innerText;
  // 행사정보 - 행사이름
  let eventNameSelect = document.getElementById("select-event-day");
  let eventNameSelectValue =
    eventNameSelect.options[eventNameSelect.selectedIndex].value;
  // 행사일시 - 날짜
  let dateValue =
    document.querySelector("#calYyyyMM .year").innerText +
    "-" +
    document.querySelector("#calYyyyMM .month").innerText +
    "-" +
    document.querySelector("td.select").innerText;
  ...
  let timeSelectValue = timeSelect.options[
    timeSelect.selectedIndex
  ].innerText.substr(0, 18);
  //총 인원
  let totalPerson = document.querySelector(".total-person").innerText;
  //예매번호 랜덤발생
  let reserveNumberValue = `RS${String(
  ...
  document.querySelector(
    ".reserved-event"
  ).innerText = `${eventSelectValue} : ${eventNameSelectValue}`;
  // 행사 일시
  document.querySelector(
    ".reserved-event-date"
  ).innerText = `${dateValue}${dayValue} ${timeSelectValue}`;
  // 총 인원
  ...
  let payMethodSelectValue =
    payMethodSelect.options[payMethodSelect.selectedIndex].innerText;
  ...
  let data = {
    ...
    totalPrice: document.querySelector(".reservation-total-price .total-price")
      .innerText,
...
modalConfirmReserveBtn.addEventListener("click", (e) => {
  ...
  const reservationNumber =
    document.querySelector(".reserved-number").innerText;
  //예매 번호 >>>>> 민욱수정
  e.preventDefault();
...
const modalCloseBtn = document.querySelector(
  ".booking-modal-content .modal-buttons button.close"
);
